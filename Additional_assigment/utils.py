...
def load_data(filename):
    ...
    train_data = (train_data[0].astype(np.float16), train_data[1])
    val_data = (val_data[0].astype(np.float16), val_data[1])
    test_data = (test_data[0].astype(np.float16), test_data[1])
...
def plot_val_loss_repeated(results, dirname=None):
    experiment_results = []

    for result in results:
        val_loss_data = []
        for log_data in result['log_data']:
            val_loss_data.append(log_data['loss_epoch']['val'])
        experiment_results.append(np.array(val_loss_data))

    experiment_results = np.mean(experiment_results, axis=0)

    filename = 'loss_val'
    plot_data(experiment_results, result['label'], x_label='epoka', y_label='funkcja kosztu',
              filename=filename, dirname=dirname, leg_loc='upper right')
...
def plot_val_accuracy_repeated(results, dirname=None):
    experiment_results = []
    ...
    for result in results:
        ...
        for log_data in result['log_data']:
            val_acc_data.append(log_data['accuracy']['val'])
        experiment_results.append(np.array(val_acc_data))

    experiment_results = np.mean(experiment_results, axis=0)

    filename = 'acc_val'
    plot_data(experiment_results, result['label'], x_label='epoka', y_label='skuteczność',
              filename=filename, dirname=dirname, leg_loc='lower right')
